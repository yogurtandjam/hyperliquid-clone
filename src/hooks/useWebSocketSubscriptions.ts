...
export function useWebSocketSubscriptions(props: WebSocketSubscriptionsProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log("Creating new webData2 subscription for user:", user?.wallet?.address);
        
        subscriptionsRef.current.webData2 = await subscriptionClient.webData2(
          {
            user: (user?.wallet?.address ?? "0x0000000000000000000000000000000000000000") as Address,
          },
          (payload) => {
              ...
              setBalanceData({
                ...
                unrealizedPnl:
                  clearingHouse.assetPositions?.reduce((total, nextAsset) => {
                    const pnl = toNumSafe(nextAsset.position?.unrealizedPnl) || 0;
                    return total + pnl;
                  }, 0) || 0,
              });
              ...
              const positions =
                clearingHouse.assetPositions
                  ?.map((assetPos) => {
                    ...
                    const szDecimals = assetsMap[position.coin]?.szDecimals || 0;

                    // Skip positions with no size
                    if (!position.szi || toNumSafe(position.szi) === 0) return null;

                    return {
                      coin: position.coin,
                      szi: priceToWire(position.szi, "perp", szDecimals),
                      entryPx: priceToWire(position.entryPx, "perp", szDecimals),
                      unrealizedPnl: formatters.formatPriceChange(position.unrealizedPnl),
                      liquidationPrice: priceToWire(position.liquidationPx ?? 0, "perp", szDecimals),
            ...
            const mappedOrders = openOrders.map((order) => {
              ...
              return {
                ...
                filled: (toNumSafe(order.origSz) - toNumSafe(order.sz))?.toString(),
            ...
            setMarketData((prev) => {
              ...
              for (let i = 0; i < arr.length; i++) {
                ...
                const abs = mark != null && prevDayPx != null ? mark - prevDayPx : 0;
                const pct = prevDayPx != null && prevDayPx !== 0 ? (abs / prevDayPx) * 100 : 0;
                ...
                next[symbol] = {
                  ...
                  volume24h: volUsd != null ? fmtUsd2(volUsd) : prevRow.volume24h,
                };
              }
              return next;
            });
  ...
  }, [ready, availableAssets, setMarketData, setIsConnected, setBalanceData, setPositions, setOpenOrders, user?.wallet?.address, assetsMap]);
  ...
  useEffect(() => {
    const setupSymbolSubscriptions = async () => {
      ...
      try {
            ...
            if (data && data.levels) {
              ...
              const spreadPerc =
                parseFloat(bestBid) > 0
                  ? (((parseFloat(bestAsk) - parseFloat(bestBid)) / parseFloat(bestBid)) * 100).toFixed(4)
                  : "0";
              ...
              const processedAsks = (Array.isArray(asks) ? asks : [])
                ...
                .map((level, index: number) => {
                  ...
                  const runningTotal = (Array.isArray(asks) ? asks : [])
                    .slice(0, index + 1)
                    .reduce((sum: number, lvl) => {
                      const levelSize = Array.isArray(lvl) ? lvl[1] : lvl.sz || "0";
                      return sum + parseFloat(levelSize);
                    }, 0);
                  ...
                  return {
                    price,
                    size,
                    total: priceToWire(runningTotal, "perp", selectedAsset?.szDecimals),
                  };
                });
        ...
        console.error(`❌ Error setting up subscriptions for ${selectedSymbol}:`, error);
      }
    };
  ...
  }, [selectedSymbol, selectedAsset?.szDecimals, setOrderBook, setRecentTrades]);
